  ...
  <el-container>
    ...
    <el-main>
        ...
        <el-row>
          <el-col :span="8">
            <el-form-item label="挂单状态">
              <el-select v-model="searchForm.state" placeholder="挂单状态">
                <el-option
      ...
      <el-table v-loading="table_loading" :data="list" :row-class-name="tableRowClassName" @sort-change="sortChange">
        ...
        <el-table-column column-key="a_type" prop="a_type" label="币种" />
        <el-table-column prop="price" label="价格(USDT)">
          <template slot-scope="scope">
            <span>{{ scope.row.price | floatNormalDisplay }}</span>
          </template>
        </el-table-column>
        <el-table-column prop="amount_max" label="挂单数量" />
        <el-table-column prop="amount_cur" label="成交数量" />
        <el-table-column prop="handle_status" label="状态">
          <template slot-scope="scope">
            <el-tag class="state-tag" :type="orderStatus[scope.row.handle_status].type" size="small" effect="dark"> {{ orderStatus[scope.row.handle_status].label }} </el-tag>
          </template>
        </el-table-column>
