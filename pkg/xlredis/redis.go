...
func (i *Redis) SetNX(ctx context.Context, key string, value interface{}, expiration time.Duration) (bool, error) {
	return i.Client.SetNX(ctx, i.BuildKey(key), value, expiration).Result()
}
...
func (i *Redis) IncrByExpire(ctx context.Context, key string, value float64, expire time.Duration) (float64, error) {
	return i.Client.Eval(ctx, incrByExpScript, []string{i.BuildKey(key)}, value, expire.Seconds()).Float64()
}
...
func (i *Redis) TTL(ctx context.Context, key string) *redis.DurationCmd {
	return i.Client.TTL(ctx, i.BuildKey(key))
}
